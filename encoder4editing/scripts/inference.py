...
# def generate_inversions(args, g, latent_codes, is_cars):
#     print('Saving inversion images')
#     inversions_directory_path = os.path.join(args.save_dir, 'base')
#     os.makedirs(inversions_directory_path, exist_ok=True)
#     #######逐层更新w，代码有待优化，目前可以正常工作#########
#     #######原始文件建议为1024*1024，如果不是建议Resize或者转换；使用的模型为预训练的FFHQ Inversion#########
#     #######每次重新调用inference.py前，建议先把结果目录的latents.pt删除，否则该缓存文件会影响计算#########
#     all_codes = [] 
#     #######正序计算A->B的逐层转化效果，建议原始文件从0开始命名。0.jpg->1.jpg的第0层转化效果在0_1_0目录下，第一个0表示0.jpg,1表示1.jpg,最后的数字0表示层数#########
#     for i in range(args.n_sample):
#         imgs, _ = g([latent_codes[i].unsqueeze(0)], input_is_latent=True, randomize_noise=False, return_latents=True)
#         if is_cars:
#             imgs = imgs[:, :, 64:448, :]
#         save_image(imgs[0], inversions_directory_path, str(i))
#         index = 0
#         for c in all_codes:
#             sub_directory_path = os.path.join(args.save_dir, f'{i}_{index}')
#             os.makedirs(sub_directory_path, exist_ok=True)
#             latent_code_clone = latent_codes[i].clone()
#             for j in range(c.shape[0]):
#                 latent_code_clone[j] = c[j]
#                 imgs, _ = g([latent_code_clone.unsqueeze(0)], input_is_latent=True, randomize_noise=False, return_latents=True)
#                 if is_cars:
#                     imgs = imgs[:, :, 64:448, :]
#                 # save_image(imgs[0], sub_directory_path, f"{i}_{index}_{j}")
#                 save_image(imgs[0], sub_directory_path, "{}_{}_{:>02}".format(i, index, j))
#                 print("{}_{}_{:>02}".format(i, index, j))
#             index += 1
#         all_codes.append(latent_codes[i])
#     all_codes = [] 
#     #######逆序计算B->A的逐层转化效果，建议原始文件从0开始命名。0.jpg->1.jpg的第0层转化效果在0_1_0目录下，第一个0表示0.jpg,1表示1.jpg,最后的数字0表示层数#########
#     for i in range(args.n_sample-1, -1, -1):
#         imgs, _ = g([latent_codes[i].unsqueeze(0)], input_is_latent=True, randomize_noise=False, return_latents=True)
#         if is_cars:
#             imgs = imgs[:, :, 64:448, :]
#         save_image(imgs[0], inversions_directory_path, str(i))
#         index = args.n_sample - 1
#         for c in all_codes:
#             sub_directory_path = os.path.join(args.save_dir, f'{i}_{index}')
#             os.makedirs(sub_directory_path, exist_ok=True)
#             latent_code_clone = latent_codes[i].clone()
